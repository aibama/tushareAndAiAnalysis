...
def get_existing_dates(ts_code: str) -> set:
    ...
    try:
        ...
        result = db.DBSession.query(etf_daily_info.trade_date).filter(
            etf_daily_info.ts_code == ts_code
        ).all()
    ...
    try:
        ...
        for x in df.to_dict('records'):
            record = {
                ...
                'open': to_none(x.get('open')),
                'high': to_none(x.get('high')),
                'low': to_none(x.get('low')),
                'close': to_none(x.get('close')),
                'pre_close': to_none(x.get('pre_close')),
                'change': to_none(x.get('change')),
                'pct_chg': to_none(x.get('pct_chg')),
                'vol': to_none(x.get('vol')),
                'amount': to_none(x.get('amount')),
                'last_update_time': datetime.now()
            }
            records.append(record)
        stmt = insert(etf_daily_info).values(records)
